...
def sanitize_dict(data_dict, allow_html_fields=None):
    ...
    for key, value in data_dict.items():
        if isinstance(value, str):
            ...
            sanitized[key] = [
                (
                    sanitize_input(item, allow_html=key in allow_html_fields)
...
def validate_image_file(file_storage, max_size_mb=2):
    """
    Validate uploaded image file using Pillow
    ...
    # Verify file content is a valid image using Pillow
    try:
        # Open the image and check its format
        image = Image.open(file_storage)
        image.verify()  # Verify integrity without fully decoding

        # Reset file pointer after verify (verify() may close the file)
        file_storage.seek(0)

        # Optionally, ensure the format matches the extension
        # (PIL returns uppercase format names like 'JPEG', 'PNG')
        fmt = image.format.lower() if image.format else ""
        if fmt == "jpeg" and "jpg" in allowed_extensions:
            fmt = "jpg"  # Normalize to match extension check
        if fmt not in allowed_extensions and fmt != "jpeg":
            return False, f"File content does not match allowed types. Detected: {fmt}"
